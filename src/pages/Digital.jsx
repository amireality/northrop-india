...
const services = [
  {
    ...
    img: "/business-transformation1.jpeg",
      ...
      <main className="digital-section bg-[#fcf9f8] text-[#002B5C] font-sans overflow-x-hidden ">
        ...
        <section className="relative min-h-[85vh] flex items-center justify-center">
          ...
          <div className="relative z-10 max-w-[1280px] mx-auto px-8 lg:px-16 text-center">
            <div className="inline-block mb-6 px-4 py-1 bg-[#002B5C]/5 text-[#ffff] text-[12px] font-semibold uppercase tracking-[0.3em] rounded-full">Precision Engineering</div>
            <h1 className="font-h1 text-4xl lg:text-6xl text-[#ffff] mb-8 max-w-4xl mx-auto leading-tight font-bold">The Architecture of Global Excellence</h1>
            <p className="text-[18px] text-[#ffff] max-w-2xl mx-auto mb-12 leading-relaxed">Bridging the critical gap between high-level corporate strategy and rigorous technical engineering.</p>
            <div className="flex flex-col md:flex-row gap-4 justify-center">
              <Link to="/contact">
                <button className="px-10 py-5 bg-[#ffff] text-[#002B5C] text-[12px] font-semibold uppercase tracking-widest hover:shadow-lg transition-all active:scale-95 w-full md:w-auto">Executive Briefing</button>
              </Link>
              <Link to="/contact">
                <button className="px-10 py-5 border border-[#ffff]/20 bg-white/50 backdrop-blur text-[#ffff] uppercase tracking-widest hover:border-[#ffff] transition-all active:scale-95 w-full md:w-auto">Technical Specs</button>
              </Link>
            </div>
          </div>
        </section>
            ...
            {services.map((svc, idx) => (
              <Link key={idx} to={svc.link} className="group relative aspect-square bg-white border border-[#002B5C]/10 overflow-hidden card-glow transition-all duration-500 cursor-pointer block">
                <img 
                  ...
                  src={svc.img} 
                />
                {/* <div className="absolute inset-0 bg-gradient-to-t from-white vi/a-white/80 to-transparent"></div> */}
                <div className="absolute bottom-0 p-8">
                  <h3 className="font-h3 text-xl mb-2 text-[#ffff] font-semibold">{svc.title}</h3>
                  <p className="text-[#ffff] text-sm">{svc.desc}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <section className="py-24 text-center relative overflow-hidden bg-white">
          ...
          <div className="max-w-[1280px] mx-auto px-8 lg:px-16 relative z-10">
            ...
            <Link to="/contact">
              <button className="px-12  cursor-pointer py-6 bg-[#002B5C] text-white text-[12px] font-semibold uppercase tracking-widest hover:shadow-xl transition-all active:scale-95 text-lg">Initiate Consultation</button>
            </Link>
          </div>
        </section>
      </main>
